...
In partnership with Firebase, we're making the public Vibe.fyi data available in near real time. Firebase enables easy access from Android, iOS, and the web. Servers aren't left out.
...
The type of item. One of "post", "comment", or "poll".
...
The comment's parent: either another comment or the relevant post.
...
The post's score, or the votes for a poll option.

title
	

The title of the post or poll. HTML.
...
In the case of posts or polls, the total comment count.
...
List of the user's posts, polls, and comments.
